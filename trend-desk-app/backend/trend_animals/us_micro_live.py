...
def bitget_us_universe(*, pool: dict, root_assets: tuple[str, ...]) -> list[dict]:
    """从公开交集存档提取严格匹配的 Bitget 美股范围。

    成员归档日期是身份来源，不是每日趋势快照日期。这样一份经批准的覆盖
    存档可以在后续数据日复用，避免把 ``all_basic=1`` 覆盖查询误做成日任务。
    """
    rows: list[dict] = []
    seen_tm_ids: set[int] = set()
    for row in pool.get("candidates") or []:
        if not isinstance(row, dict) or row.get("root_asset") not in root_assets:
            continue
...
def bitget_us_stock_universe(pool: dict) -> list[dict]:
    """首版实盘范围：严格匹配的 Bitget 美股个股。"""
    return bitget_us_universe(pool=pool, root_assets=("美股",))


def bitget_us_etf_observation_universe(pool: dict) -> list[dict]:
    """美国 ETF 仍归档、计数和展示，但首版不进入买入计划。"""
    return bitget_us_universe(pool=pool, root_assets=("美国ETF",))


def build_signal_scan_plan(*, pool: dict, billing: list[dict], expected_as_of_date: str,
                           root_assets: tuple[str, ...] = ("美股",)) -> dict:
    """构造最小字段、最多 300 行一批的计费计划，但绝不发起快照请求。"""
    universe = bitget_us_universe(pool=pool, root_assets=root_assets)
    ...
    return {
        ...
        "root_assets": list(root_assets),
        "membership_as_of_dates": sorted({str(row.get("asOfDate") or "") for row in universe}),
        ...
        "membership_date_note": (
            "成员归档日期仅表示趋势动物覆盖身份的来源；付费快照行必须匹配 as_of_date。"
            "根节点每日更新不自动触发全量成员覆盖刷新。"
        ),
        "execution_guard": (
...
def classify_signal_rows(*, pool: dict, snapshot_rows: list[dict], expected_as_of_date: str,
                         entry_max_days: int = DEFAULT_ENTRY_MAX_RIGHT_SIDE_DAYS,
                         root_assets: tuple[str, ...] = ("美股",)) -> dict:
