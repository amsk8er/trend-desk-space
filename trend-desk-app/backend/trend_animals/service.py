...
def ledger_rows_after(client, before: set[tuple]) -> list[dict] | None:
    """Return new ledger rows with one post-run request.

    The balance endpoint is free but limited to one request per second.  Keeping
    the raw delta available lets callers attribute costs to stages without
    issuing several back-to-back ledger requests.
    """
    if not before:
        return None
    try:
    ...
    return [
        row for row in rows
        if isinstance(row, dict) and ledger_signature(row) not in before
    ]


def ledger_rows_cost(rows: list[dict] | None) -> float | None:
    if rows is None:
        return None
    total = 0.0
    found = False
...
def ledger_delta(client, before: set[tuple]) -> float | None:
    return ledger_rows_cost(ledger_rows_after(client, before))
