...
TREND_ANIMALS_DAILY_AUTO_BUDGET = float(os.getenv(
    "TREND_ANIMALS_DAILY_AUTO_BUDGET", "5.00"))
...
# --- 美股右侧资产研究页 -----------------------------------------------------
# 与“美股手工”执行规则完全隔离；所有真实趋势动物读取仍需页面预算确认。
US_RIGHT_SIDE_ENABLED = os.getenv("US_RIGHT_SIDE_ENABLED", "true").lower() == "true"
US_RIGHT_SIDE_LEASE_MINUTES = int(os.getenv("US_RIGHT_SIDE_LEASE_MINUTES", "30"))
US_RIGHT_SIDE_CURSOR_SECRET = (
    os.getenv("US_RIGHT_SIDE_CURSOR_SECRET")
    or os.getenv("TREND_DESK_ACCESS_KEY_SHA256")
    or "trend-desk-local-us-right-side-cursor-v1"
)
...
# H5 只读兼容与 A 股行业 Top-N 验证共用的 Wind MCP 配置；H6 不依赖 Wind。
WIND_MCP_TIMEOUT_S = float(os.getenv("WIND_MCP_TIMEOUT_S", "60"))
INDUSTRY_WIND_TOP_N = max(5, min(int(os.getenv("INDUSTRY_WIND_TOP_N", "8")), 10))
WIND_VALIDATION_MODE = os.getenv("WIND_VALIDATION_MODE", "skill_cli").strip().lower()
if WIND_VALIDATION_MODE not in {"skill_cli", "direct", "off"}:
    raise RuntimeError("WIND_VALIDATION_MODE must be skill_cli, direct, or off")
WIND_SKILL_DIR = Path(os.getenv(
    "WIND_SKILL_DIR", Path.home() / ".agents" / "skills" / "wind-mcp-skill"))
WIND_SKILL_CLI_TIMEOUT_S = float(os.getenv("WIND_SKILL_CLI_TIMEOUT_S", "90"))
...
# --- OKX 只读持仓监控 -------------------------------------------------------
# 凭证只注入独立 worker；Web 进程即使没有下面三项也能读取已落库的监控结果。
OKX_MONITOR_ENABLED = os.getenv("OKX_MONITOR_ENABLED", "false").lower() == "true"
OKX_MONITOR_SHADOW_MODE = os.getenv("OKX_MONITOR_SHADOW_MODE", "true").lower() == "true"
...
OKX_API_DEMO = os.getenv("OKX_API_DEMO", "false").lower() == "true"
OKX_ACCOUNT_POLL_SECONDS = max(10, int(os.getenv("OKX_ACCOUNT_POLL_SECONDS", "15")))
OKX_HEARTBEAT_SECONDS = max(5, int(os.getenv("OKX_HEARTBEAT_SECONDS", "10")))
OKX_STALE_SECONDS = max(30, int(os.getenv("OKX_STALE_SECONDS", "60")))
OKX_LIQUIDATION_ALERT_PCT = float(os.getenv("OKX_LIQUIDATION_ALERT_PCT", "10"))
OKX_SPOT_DUST_USD = max(0.0, float(os.getenv("OKX_SPOT_DUST_USD", "0.01")))
OKX_REENTRY_COOLDOWN_MINUTES = max(1, int(os.getenv("OKX_REENTRY_COOLDOWN_MINUTES", "30")))
