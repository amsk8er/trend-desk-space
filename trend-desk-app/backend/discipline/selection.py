...
def evaluate_candidate(candidate: dict) -> dict:
    ...
    if "trend_data_current" in c:
        trend_current = bool(c.get("trend_data_current"))
        tests.append(_evidence(
            "trend_data_current", c.get("trend_as_of_date"), trend_current,
            "trend_animals",
        ))
    if "suspended" in c:
        not_suspended = not bool(c.get("suspended"))
        tests.append(_evidence("not_suspended", c.get("suspended"), not_suspended, "tushare"))
