...
def confirm_ocr_positions(s: Session, *, batch_id: str,
                          ...
                          nav: float | None = None, cash: float | None = None,
                          trend_client=None) -> dict:
    ...
    try:
        ...
        if dataset is not None:
            ...
            for position in positions:
                if not position.code and _normal_name(position.name) in code_by_name:
                    ...
                    s.add(position)
        missing = [p.name for p in positions if not _bare_code(p.code)]
        if missing:
        ...
        holding_reconciliation = None
        if dataset is not None and trend_client is not None:
            from backend.discipline.daily_data import reconcile_account_holding_codes
            holding_reconciliation = reconcile_account_holding_codes(
                s, dataset=dataset, positions=positions, trend_client=trend_client)
        ...
        if nav is not None or cash is not None:
            ...
            if dataset is not None:
                ...
                if holding_reconciliation and holding_reconciliation["backfilled"]:
                    codes = "、".join(holding_reconciliation["codes"])
                    out["message"] += (
                        f"已发现收藏夹漏掉 {holding_reconciliation['backfilled']} 项"
                        f"实际持仓（{codes}），并补齐趋势温度与强度。"
                    )
            else:
                out["message"] = (
