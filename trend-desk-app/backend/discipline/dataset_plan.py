...
def _snapshot_tags(row: TrendDailySnapshot) -> list[str]:
    """Keep source labels visible without inventing UI-only categories."""
    raw = row.raw_payload or {}
    values: list[object] = []
    for key in ("tickerLabels", "tags"):
        value = raw.get(key)
...
def dataset_context(s: Session, dataset_id: str) -> dict:
    ...
    for membership_type in ("warm_to_hot_stock", "warm_to_hot_etf"):
        for tm_id in members.get(membership_type, []):
            ...
            embedded_sector_temperature = (
                (row.raw_payload or {}).get("industryTrendTemperatureCurr")
            )
            ...
            candidates.append({
                ...
                "trend_as_of_date": row.as_of_date,
                "trend_data_current": row.as_of_date == dataset.trade_date,
                "suspended": bool(fact.suspended) if fact else False,
                "tags": _snapshot_tags(row),
                "temperature_prev": row.temperature_prev,
                "temperature_curr": row.temperature_curr,
                "phase": row.phase,
                "sector": row.industry_name,
                # minimum-fields-v2 直接随个股购买所属行业温度；旧数据集仍回退行业快照。
                "sector_temperature": (
                    embedded_sector_temperature
                    if embedded_sector_temperature is not None
                    else (sector.temperature_curr if sector else None)
                ),
    ...
    for tm_id in members.get("holding", []):
        row = snap_by_tm.get(tm_id)
        if row is None or not row.code or row.as_of_date != dataset.trade_date:
            continue
...
def ensure_signal_plan(s: Session, dataset_id: str) -> dict:
    ...
    for signal in ctx["signals"]:
        ...
        evidence.update({"signal_only": True, "tags": signal.get("tags") or [],
                         "phase": signal.get("phase"), "price": signal.get("price")})
        s.add(TradePlanItem(
