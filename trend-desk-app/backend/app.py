...
from backend.api.okx_monitor import router as okx_monitor_router
...
async def lifespan(app: FastAPI):
    ...
    from backend.us_manual.scheduler import (
        scheduler_enabled as us_scheduler_enabled,
        scheduler_loop as us_scheduler_loop,
    )

    scheduler_task = None
    us_scheduler_task = None
    bootstrap_task = None
    app.state.database_ready = False

    async def bootstrap_database_and_scheduler():
        nonlocal scheduler_task, us_scheduler_task
        try:
            ...
            if us_scheduler_enabled():
                us_scheduler_task = asyncio.create_task(us_scheduler_loop())
    ...
    try:
        ...
        if scheduler_task is not None:
            ...
            try:
                ...
                pass
        if us_scheduler_task is not None:
            us_scheduler_task.cancel()
            try:
                await us_scheduler_task
            except asyncio.CancelledError:
                pass
...
from backend.api.us_manual import router as us_manual_router  # noqa: E402
from backend.api.us_right_side import router as us_right_side_router  # noqa: E402
from backend.api.decision_events import router as decision_events_router  # noqa: E402
from backend.api.market_evidence import router as market_evidence_router  # noqa: E402
from backend.api.us_calculator import router as us_calculator_router  # noqa: E402
...
app.include_router(us_manual_router)
app.include_router(us_right_side_router)
app.include_router(decision_events_router)
app.include_router(market_evidence_router)
app.include_router(us_calculator_router)
app.include_router(okx_monitor_router)
