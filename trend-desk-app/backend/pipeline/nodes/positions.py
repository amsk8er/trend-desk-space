...
def _retryable_ocr_error(error: Exception) -> bool:
    """Whether repeating a vision request can reasonably recover.

    Provider gateways commonly surface model stalls as 5xx responses (AIGoCode
    currently returns 524).  Retrying deterministic 4xx rejections only wastes
    quota and keeps the UI spinning, so fail those closed immediately.
    """
    from backend.llm.base import LLMNonRetryableError
...
def _public_ocr_error(error: Exception) -> str:
    """Return an actionable message without leaking provider URLs or bodies."""
    if isinstance(error, httpx.HTTPStatusError):
        status = error.response.status_code
        if status == 524:
            return "视觉服务处理截图超时（HTTP 524），已自动重试；请稍后再试或切换 OCR 引擎"
        if status == 429:
            return "视觉服务请求过于频繁（HTTP 429），已自动重试；请稍后再试"
        if status in {401, 403}:
            return f"视觉服务认证或额度不可用（HTTP {status}），请检查生产配置"
        if status == 413:
            return "截图体积超过视觉服务限制（HTTP 413），请裁剪无关区域后重试"
        if status >= 500:
            return f"视觉服务暂时不可用（HTTP {status}），已自动重试；请稍后再试"
        return f"视觉服务拒绝了截图请求（HTTP {status}）"
    if isinstance(error, (TimeoutError, httpx.TimeoutException)):
        return "视觉服务处理截图超时，已自动重试；请稍后再试或切换 OCR 引擎"
    if isinstance(error, httpx.RequestError):
        return "无法连接视觉服务，已自动重试；请检查网络后再试"
    return f"截图识别失败（{type(error).__name__}）"
...
def upsert_positions(s: Session, *, batch_id: str, rows: list[dict]) -> None:
    # 截图明确印出代码时逐行照抄；没有时 code=None，OCR 禁止脑补。
    # 空代码可由已有趋势动物持仓成员按名称回填；已识别代码用于反查收藏夹漏项。
...
async def run_positions_node(*, engine, batch_id: str, image_paths: list[str],
                             ...
                             max_retries: int = 2, timeout_s: int = 180,
                             retry_delay_s: float = 2.0) -> dict:
    ...
    for i, img in enumerate(image_paths):
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                retryable = _retryable_ocr_error(e)
                if not retryable or attempt >= max_retries:
                    failed.append({"image": img, "error": _public_ocr_error(e)})
                    break
                # Immediate retries hit the same overloaded upstream window.
                # A short exponential delay is enough to make transient gateway
                # failures recover without blocking the upload HTTP request.
    ...
    with Session(engine) as s:
        all_images_failed = bool(image_paths) and len(failed) == len(image_paths)
        if not all_images_failed:
    ...
    async def _run():
        ...
        try:
            ...
            _POS_PROGRESS[batch_id] = {**_POS_PROGRESS.get(batch_id, {}),
                                       "status": "error", "error": _public_ocr_error(e)}
