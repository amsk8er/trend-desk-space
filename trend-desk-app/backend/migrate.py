...
_EXPECTED = {
    ...
    # 本地 Wind Skill 适配：行业代码映射落在稳定目录缓存，避免每天重复
    # 调用 get_index_basicinfo；旧 SQLite 库通过 ensure_columns 增量补齐。
    "industry_catalog_entry": {
        "wind_code": "VARCHAR",
        "wind_name": "VARCHAR",
        "wind_mapping_status": "VARCHAR",
        "wind_mapping_error": "VARCHAR",
        "wind_mapping_at": "DATETIME",
    },
    "us_daily_run": {
        "estimated_total_cost_cny": "NUMERIC(20, 8)",
        "actual_total_cost_cny": "NUMERIC(20, 8)",
        "daily_cost_cap_cny": "NUMERIC(20, 8)",
        ...
        "exit_status": "VARCHAR",
        "environment_status": "VARCHAR",
        "etf_mapping_status": "VARCHAR",
    },
    "us_candidate_snapshot": {
        "gate_passed": "BOOLEAN",
        "ticker_labels": "JSON",
        "trend_phase_curr": "VARCHAR",
        "observation_rank": "INTEGER",
        "reference_price_usdt": "NUMERIC(24, 8)",
        "reference_price_at": "DATETIME",
        "reference_price_source": "VARCHAR",
        "quote_status": "VARCHAR",
        "environment_id": "VARCHAR",
        "etf_identity_id": "VARCHAR",
        "etf_benchmark_evidence_id": "VARCHAR",
        "benchmark_family_id": "VARCHAR",
        "exposure_key": "VARCHAR",
        "benchmark_status": "VARCHAR",
    },
    "us_right_side_run": {
        "age_fields": "JSON",
        "age_fields_hash": "VARCHAR",
        "estimated_age_cost_cny": "NUMERIC(20, 8)",
        "approved_age_budget_cny": "NUMERIC(20, 8)",
        "actual_age_cost_cny": "NUMERIC(20, 8)",
        "age_covered_count": "INTEGER",
        "standard_target_count": "INTEGER",
        "standard_max_days": "INTEGER",
        "age_ready": "BOOLEAN",
        "strength_fields": "JSON",
        "strength_fields_hash": "VARCHAR",
        "estimated_strength_cost_cny": "NUMERIC(20, 8)",
        "approved_strength_budget_cny": "NUMERIC(20, 8)",
        "actual_strength_cost_cny": "NUMERIC(20, 8)",
        "strength_covered_count": "INTEGER",
        "strength_target_count": "INTEGER",
        "strength_max_days": "INTEGER",
        "standard_top_n": "INTEGER",
        "strength_ready": "BOOLEAN",
    },
    "us_manual_plan_item": {
        "stop_suggestion_id": "VARCHAR",
        "stop_review_id": "VARCHAR",
        "sizing_preview_id": "VARCHAR",
        "stop_anchor_type": "VARCHAR",
        "stop_anchor_date": "VARCHAR",
        "stop_evidence_json": "JSON",
        "allocation_preview_item_id": "INTEGER",
        "risk_anchor_id": "VARCHAR",
        "environment_id": "VARCHAR",
        "exit_decision_id": "VARCHAR",
        "risk_anchor_price": "NUMERIC(24, 8)",
        "risk_anchor_date": "VARCHAR",
        "anchor_loss_estimate_usdt": "NUMERIC(20, 8)",
    },
    "us_manual_plan": {"allocation_preview_id": "VARCHAR"},
    "us_position_lot": {
        "stop_source": "VARCHAR",
        "stop_suggestion_id": "VARCHAR",
        "stop_review_id": "VARCHAR",
...
def ensure_columns(engine) -> list[str]:
    ...
    with Session(engine) as s:
        ...
        if "industry_catalog_entry.wind_mapping_status" in added:
            s.exec(text(  # type: ignore[call-arg]
                "UPDATE industry_catalog_entry SET wind_mapping_status = 'unmapped' "
                "WHERE wind_mapping_status IS NULL"))
        ...
        if "us_candidate_snapshot.benchmark_status" in added:
            s.exec(text(  # type: ignore[call-arg]
                "UPDATE us_candidate_snapshot SET benchmark_status = 'not_applicable' "
                "WHERE benchmark_status IS NULL"))
        if "us_candidate_snapshot.gate_passed" in added:
            s.exec(text(  # type: ignore[call-arg]
                "UPDATE us_candidate_snapshot SET gate_passed = 0 "
                "WHERE gate_passed IS NULL"))
        if "us_candidate_snapshot.ticker_labels" in added:
            s.exec(text(  # type: ignore[call-arg]
                "UPDATE us_candidate_snapshot SET ticker_labels = '[]' "
                "WHERE ticker_labels IS NULL"))
        if "us_right_side_run.strength_fields" in added:
            from backend.us_right_side.contracts import (
                STANDARD_ASSET_FIELDS, STRENGTH_FIELDS, field_set_hash,
            )
            strength_json = '["trendStrengthLocalCurr"]'
            standard_json = '["industryTmId","trendTemperatureCurr","trendPhaseCurr"]'
            s.exec(text(  # type: ignore[call-arg]
                "UPDATE us_right_side_run SET "
                "strength_fields = :strength_fields, strength_fields_hash = :strength_hash, "
                "strength_covered_count = COALESCE(strength_covered_count, 0), "
                "strength_target_count = COALESCE(standard_target_count, 0), "
                "strength_max_days = COALESCE(standard_max_days, 30), "
                "standard_top_n = 100, strength_ready = 0 "
                "WHERE strength_fields IS NULL"), params={
                    "strength_fields": strength_json,
                    "strength_hash": field_set_hash(STRENGTH_FIELDS),
                })
            s.exec(text(  # type: ignore[call-arg]
                "UPDATE us_right_side_run SET "
                "standard_fields = :standard_fields, standard_fields_hash = :standard_hash, "
                "standard_target_count = 0, estimated_standard_cost_cny = NULL, "
                "approved_standard_budget_cny = NULL, status = 'awaiting_strength_budget' "
                "WHERE standard_covered_count = 0 AND standard_ready = 0 AND age_ready = 1"),
                params={
                    "standard_fields": standard_json,
                    "standard_hash": field_set_hash(STANDARD_ASSET_FIELDS),
                })
        for table in ("us_manual_plan_item", "us_position_lot"):
            marker = f"{table}.stop_evidence_json"
            if marker in added:
                s.exec(text(  # type: ignore[call-arg]
                    f"UPDATE {table} SET stop_evidence_json = '{{}}' "
                    "WHERE stop_evidence_json IS NULL"))
        s.commit()
    return added
