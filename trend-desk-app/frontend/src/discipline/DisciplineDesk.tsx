...
import {
  ...
  runAutomationNow, sendAutomationTestEmail, updateFeeSchedule, getUsManualCapabilities,
  getUsRightSideCapabilities, getOkxMonitorCapabilities,
...
import UsRiskCalculator from "./UsRiskCalculator";
...
type Page = "desk" | "industry" | "us" | "okx" | "us-right-side" | "calculator" | "review" | "data";
...
const pages: { id: Page; label: string; mark: string }[] = [
  { id: "desk", label: "A股交易台", mark: "01" },
  { id: "industry", label: "行业主线", mark: "02" },
  { id: "us", label: "美股手工", mark: "03" },
  { id: "okx", label: "OKX持仓", mark: "04" },
  { id: "us-right-side", label: "美股右侧", mark: "05" },
  { id: "calculator", label: "仓位计算器", mark: "06" },
  { id: "review", label: "复盘", mark: "07" },
  { id: "data", label: "数据与规则", mark: "08" },
];
...
function initialPage(): Page {
  ...
  if (requested && pageIds.has(requested)) return requested;
  if (params.has("sort_by") || params.has("us_sort")) return "us-right-side";
  return "desk";
}
...
export default function DisciplineDesk({
  ...
  const usCapabilitiesQ = useQuery({
    queryKey: ["us-manual-capabilities"], queryFn: getUsManualCapabilities, retry: false,
  });
  const usRightSideCapabilitiesQ = useQuery({
    queryKey: ["us-right-side-capabilities"], queryFn: getUsRightSideCapabilities, retry: false,
  });
  const okxCapabilitiesQ = useQuery({
    queryKey: ["okx-monitor-capabilities"], queryFn: getOkxMonitorCapabilities, retry: false,
  });
  const isIndependentPage = page === "us" || page === "okx" || page === "us-right-side" || page === "calculator";
  useEffect(() => {
    if (page === "us" && usCapabilitiesQ.data && !usCapabilitiesQ.data.enabled) setPage("desk");
    if (page === "us-right-side" && usRightSideCapabilitiesQ.data && !usRightSideCapabilitiesQ.data.enabled) setPage("desk");
  }, [page, usCapabilitiesQ.data, usRightSideCapabilitiesQ.data]);
  useEffect(() => {
    const url = new URL(window.location.href);
    if (page === "desk") url.searchParams.delete("page");
    else url.searchParams.set("page", page);
    window.history.replaceState(null, "", url);
  }, [page]);
  const dataQ = useQuery({
    queryKey: ["discipline-data-today"], queryFn: getDisciplineTodayData,
    enabled: !isIndependentPage,
    refetchInterval: q => ["checking", "fetching", "waiting_retry"].includes(q.state.data?.status ?? "") ? 30_000 : false,
  });
  const plans = useQuery({ queryKey: ["discipline-plans"], queryFn: getDisciplinePlans, enabled: !isIndependentPage });
  useEffect(() => {
    if (!planId && plans.data?.[0]?.plan_id) setPlanId(plans.data[0].plan_id);
  }, [planId, plans.data]);
  const planQ = useQuery({
    queryKey: ["discipline-plan", planId], queryFn: () => getDisciplinePlan(planId), enabled: !!planId && !isIndependentPage,
  });
  const plan = planQ.data ?? null;
  const versionQ = useQuery({ queryKey: ["discipline-version"], queryFn: getDisciplineVersion, enabled: !isIndependentPage });
  ...
  const confirm = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["decision-events", planId] });
    },
  });
  ...
  return <div className="discipline-shell" data-testid="discipline-desk">
    <aside className="discipline-sidebar">
      <div className="discipline-brand">
        ...
        <div><b>Trend Desk</b><small>纪律先于观点</small></div>
      </div>
      <nav>{pages.filter(item => {
        if (item.id === "us") return usCapabilitiesQ.data?.enabled;
        if (item.id === "us-right-side") return usRightSideCapabilitiesQ.data?.enabled;
        if (item.id === "okx") return okxCapabilitiesQ.data?.read_only;
        return true;
      }).map(item => <button
        key={item.id}
    ...
    <main className="discipline-main">
      {!isIndependentPage && <header className="discipline-topbar">
      ...
      {!isIndependentPage && manualCollect.isError && <ErrorLine error={manualCollect.error as Error} />}
      {page === "us" && <UsManualDesk />}
      {page === "okx" && <OkxHoldingsPage />}
      {page === "us-right-side" && <UsRightSidePage />}
      {page === "calculator" && <UsRiskCalculator />}
      {page === "industry" && <IndustryHeatPage />}
...
function TradingDesk({
  ...
  useEffect(() => {
    if (dataset?.status !== "awaiting_budget" || !dataset.estimated_cost) return;
    const minimum = Math.ceil(dataset.estimated_cost * 100) / 100;
    setBudget(current => Number(current) + 1e-9 >= minimum ? current : minimum.toFixed(2));
  }, [dataset?.status, dataset?.estimated_cost]);
  ...
  return <section data-testid="page-desk" className="discipline-page trading-desk">
    <div className="desk-hero">
      <div>
        <small>DAILY DISCIPLINE DESK</small>
        <h1>明日行动清单</h1>
        <p>卖出优先，买入受容量约束；关键指标已前置，点击行可看决策详情。</p>
      </div>
    ...
    <MarketEvidenceSummary tradeDate={dataset?.trade_date ?? plan?.signal_date} />

    <div className="action-workbench">
      <section className="desk-panel action-ledger">
        <div className="panel-head loud">
          <div><small>A SHARE · PRIMARY</small><h2>行动清单</h2></div>
      ...
      <aside className="desk-rail">
        <section className="desk-panel tally-card">
          <div className="tally-title"><b>行动摘要</b></div>
    ...
    {(dataset?.error_message || dataset?.status === "awaiting_budget") && <div className="control-bar">
      ...
      {dataset?.status === "awaiting_budget" && <>
        ...
        <button className="desk-button orange" disabled={approve.isPending} onClick={() => approve.mutate()}>批准并采集</button>
      </>}
    </div>}
...
function ActionGroup({
  ...
  >
    ...
    <ActionMetrics item={item} candidate={candidateMap.get(bareCode(item.instrument_id))} />
...
export function ActionMetrics({ item, candidate }: { item: DisciplinePlanItem; candidate?: DisciplineCandidate }) {
  const evidence = item.rule_evidence ?? {};
  const previous = candidate?.temperature_prev ?? evidenceValue(evidence, "temperature_prev");
  const current = candidate?.temperature_curr ?? evidenceValue(evidence, "temperature_curr");
  const strength = candidate?.strength ?? numericEvidenceValue(evidence, "strength");
  const strengthChange = candidate?.strength_change ?? evidenceValue(evidence, "strength_change");
  const phase = candidate?.phase ?? evidenceValue(evidence, "phase");
  const price = candidate?.price ?? numericEvidenceValue(evidence, "price");
  const tags = decisionTags(item, candidate);
  return <span className="action-metrics" aria-label="关键指标">
    <Metric label="温度" value={`${previous ?? "—"}→${current ?? "—"}`} />
    <Metric label="强度" value={`${formatNumber(strength)}${strengthChange ? ` ${strengthChange}` : ""}`} />
    <Metric label="节气" value={String(phase ?? "—")} />
    <Metric label="价格" value={price == null ? "—" : `¥${formatNumber(price)}`} />
    <Metric label="标签" value={tags.join(" · ") || "—"} wide />
  </span>;
}

function Metric({ label, value, wide = false }: { label: string; value: string; wide?: boolean }) {
  return <span className={`action-metric${wide ? " wide" : ""}`}><small>{label}</small><b>{value}</b></span>;
}
...
function CandidateLane({
  ...
  return <section className={`candidate-lane ${tone}`}>
    ...
    >
      ...
      <span className="candidate-decision-wrap">
        ...
        <small>{row.tags?.join(" · ") || "无趋势标签"}</small>
      </span>
...
function DetailDrawer({ payload, onClose }: { payload: DrawerPayload; onClose: () => void }) {
  ...
  return <div className="drawer-layer" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <aside className={`evidence-drawer ${tone}`} role="dialog" aria-modal="true" aria-labelledby="drawer-title">
      <header>
        <div><small>{item ? "行动详情" : "筛选详情"}</small><h2 id="drawer-title">{title}</h2><span className="mono">{code}</span></div>
      ...
      <div className="drawer-body">
        ...
        {item && <DecisionSummary item={item} candidate={candidate} />}
        {item && <details className="drawer-audit">
          <summary>审计信息</summary>
          <EvidenceSection title="数据日期" data={item.source_dates} />
          <EvidenceSection title="数据来源" data={item.data_sources} />
        </details>}
      </div>
...
function DecisionSummary({ item, candidate }: { item: DisciplinePlanItem; candidate?: DisciplineCandidate }) {
  const tags = decisionTags(item, candidate);
  const facts = [
    ["动作", sideLabel[item.side] ?? item.side],
    ["目标", actionText(item)],
    ["标签", tags.join(" · ") || "—"],
  ];
  return <section className="drawer-section decision-summary"><h3>决策摘要</h3><dl>
...
function evidenceValue(data: unknown, key: string): string | undefined {
  if (!data || typeof data !== "object") return undefined;
  const record = data as Record<string, unknown>;
  const direct = record[key];
  if (["string", "number"].includes(typeof direct)) return String(direct);
  for (const value of Object.values(record)) {
    if (!value || typeof value !== "object") continue;
    const nested = Array.isArray(value)
      ? value.map(entry => evidenceValue(entry, key)).find(Boolean)
      : evidenceValue(value, key);
    if (nested) return nested;
  }
  return undefined;
}

function numericEvidenceValue(data: unknown, key: string): number | undefined {
  const value = evidenceValue(data, key);
  if (value == null || value.trim() === "") return undefined;
  const parsed = Number(value);
  return Number.isFinite(parsed) ? parsed : undefined;
}

function decisionTags(item: DisciplinePlanItem, candidate?: DisciplineCandidate) {
  const evidence = item.rule_evidence ?? {};
  const saved = Array.isArray(evidence.tags) ? evidence.tags : [];
  const tags = [...(candidate?.tags ?? []), ...saved]
