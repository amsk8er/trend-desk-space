...
const pages: { id: Page; label: string; mark: string }[] = [
  ...
  { id: "us", label: "美股手工", mark: "02" },
  { id: "review", label: "复盘", mark: "03" },
  { id: "data", label: "数据与规则", mark: "04" },
];
...
export default function DisciplineDesk({
  ...
  const plans = useQuery({ queryKey: ["discipline-plans"], queryFn: getDisciplinePlans, enabled: !isUsPage });
  useEffect(() => {
    if (!planId && plans.data?.[0]?.plan_id) setPlanId(plans.data[0].plan_id);
  }, [planId, plans.data]);
  const planQ = useQuery({
    queryKey: ["discipline-plan", planId], queryFn: () => getDisciplinePlan(planId), enabled: !!planId && !isUsPage,
  });
  const plan = planQ.data ?? null;
  const versionQ = useQuery({ queryKey: ["discipline-version"], queryFn: getDisciplineVersion, enabled: !isUsPage });
  ...
  return <div className="discipline-shell" data-testid="discipline-desk">
    <aside className="discipline-sidebar">
      ...
      <nav>{pages.filter(item => item.id !== "us" || usCapabilitiesQ.data?.enabled).map(item => <button
        key={item.id}
    ...
    <main className="discipline-main">
      {!isUsPage && <header className="discipline-topbar">
      ...
      {!isUsPage && manualCollect.isError && <ErrorLine error={manualCollect.error as Error} />}
