...
function CandidateRow({ candidate, onOpen }: { candidate: UsCandidate; onOpen: () => void }) {
  ...
  return <button className={`us-candidate-row ${candidate.screen_status} anchor-${candidate.risk_anchor_status ?? "pending"}`} aria-label={`查看 ${candidate.ticker_symbol} 的筛选证据`} onClick={onOpen}>
