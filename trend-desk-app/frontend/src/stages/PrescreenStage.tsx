...
import { DEFAULT_MIN_FLOAT_MARKET_CAP_YI, DEFAULT_MIN_AMOUNT_YI, DEFAULT_ETF_MIN_AUM_YI, DEFAULT_ETF_MIN_AMOUNT_YI } from "../discipline/rules";
      ...
      <div className="toolbar">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <label style={{ fontSize: 10.5, fontWeight: 700, color: "#374151", display: "flex", alignItems: "center", gap: 3 }}
            title={`个股市值门：市值(亿)低于此值的个股被剔。留空用默认 ${DEFAULT_MIN_FLOAT_MARKET_CAP_YI} 亿。`}>
            个股市值
            <input type="number" step="10" min="0" placeholder={String(DEFAULT_MIN_FLOAT_MARKET_CAP_YI)}
          ...
          <label style={{ fontSize: 10.5, fontWeight: 700, color: "#374151", display: "flex", alignItems: "center", gap: 3 }}
            title={`个股成交额门：日成交额(亿)低于此值的个股被剔。留空用默认 ${DEFAULT_MIN_AMOUNT_YI} 亿。`}>
            个股成交额
            <input type="number" step="0.5" min="0" placeholder={String(DEFAULT_MIN_AMOUNT_YI)}
              value={stkTurnoverStr} onChange={(e) => setStkTurnoverStr(e.target.value)}
              style={{ width: 48, padding: "3px 5px", border: "2px solid #1f2937", borderRadius: 6, fontSize: 11 }} />亿
          </label>
          <label style={{ fontSize: 10.5, fontWeight: 700, color: "#374151", display: "flex", alignItems: "center", gap: 3 }}
            title={`ETF 规模门：低于此规模(亿)的 ETF 被剔（流动性/清盘风险）。留空用默认 ${DEFAULT_ETF_MIN_AUM_YI} 亿。`}>
            ETF规模
            <input type="number" step="1" min="0" placeholder={String(DEFAULT_ETF_MIN_AUM_YI)}
              value={etfAumStr} onChange={(e) => setEtfAumStr(e.target.value)}
              style={{ width: 48, padding: "3px 5px", border: "2px solid #1f2937", borderRadius: 6, fontSize: 11 }} />亿
          </label>
          <label style={{ fontSize: 10.5, fontWeight: 700, color: "#374151", display: "flex", alignItems: "center", gap: 3 }}
            title={`ETF 成交额门：日成交额(亿)低于此值的 ETF 被剔。留空用默认 ${DEFAULT_ETF_MIN_AMOUNT_YI} 亿。`}>
            ETF成交额
            <input type="number" step="0.5" min="0" placeholder={String(DEFAULT_ETF_MIN_AMOUNT_YI)}
