...
// ── 美股仓位 / 止损计算器 ──
export interface UsCalculatorBar {
  time: string;
  open: number;
  high: number;
  low: number;
...
export interface UsCalculatorMarket {
  ticker: string;
  name: string;
  exchange: string;
  currency: string;
  ...
  indicators: {
    ...
    ema200: number | null;
    atr14: number | null;
    atr_short_stop: number | null;
    atr_medium_stop: number | null;
  };
  bars: UsCalculatorBar[];
}
...
export interface IndustryHeatRow {
  heat_id?: number | null; dataset_id: string; trade_date: string;
  industry_tm_id: number; industry_name: string;
  temperature_curr: string | null; strength_curr: number | null;
  strength_change: string | null; phase_curr: string | null;
  warming_streak: number; hot_duration_days: number;
  strength_slope_5d: number | null; warm_to_hot_count: number;
  trend_score: number; trend_rank: number; trend_state: string;
  trend_evidence: Record<string, number>;
  wind_status: string; wind_score: number | null; wind_coverage: number;
  wind_metrics: Record<string, number | null | Record<string, number>>;
  wind_error: string | null; mainline_score: number; mainline_state: string;
}
export interface IndustryHeatReport {
  trade_date: string | null; dataset_id?: string;
  headline: { title: string; summary: string; confidence: string };
  rows: IndustryHeatRow[];
  coverage: { industry_count?: number; trend_complete?: number; wind_requested?: number; wind_verified?: number };
  methodology?: { trend_score: string; mainline_score: string; wind_source?: string; wind_fields: string[] };
  refresh?: { rows: number; wind: { status: string; requested: number; verified: number } };
}
export type MarketEvidenceBucket = "support" | "challenge" | "watch";
export type MarketEvidenceStatus = "ready" | "partial" | "missing" | "stale";
export type MarketEvidenceEffect = "active" | "gate" | "display_only";
export interface MarketEvidenceItem {
  id: string; bucket: MarketEvidenceBucket; label: string;
  value: unknown; display_value: string; source: string; as_of: string | null;
  status: MarketEvidenceStatus; interpretation_rule: string;
  discipline_effect: MarketEvidenceEffect; detail: string;
}
export interface MarketEvidenceReport {
  trade_date: string; dataset_id: string | null; discipline_version: string;
  headline: {
    market_temperature: string | null; environment_factor: number;
    per_position_weight: number; opening_allowed: boolean; one_liner: string;
  } | null;
  items: MarketEvidenceItem[];
  summary: Record<MarketEvidenceBucket | MarketEvidenceStatus, number>;
  generated_at: string; network_calls: 0; llm_required: false;
}
...
export type DecisionEventType =
  | "plan_generated" | "no_trade_confirmed" | "candidate_excluded"
  | "risk_blocked" | "execution_confirmed" | "execution_missed"
  | "review_completed" | "note";
export interface DecisionEvent {
  event_id: string; idempotency_key: string; trade_date: string; market: string;
  event_type: DecisionEventType; instrument_id: string | null; plan_id: string | null;
  candidate_id: string | null; reason_code: string | null; note: string | null;
  discipline_version: string | null; dataset_id: string | null; facts_hash: string | null;
  payload_json: Record<string, unknown>; created_at: string;
}
export interface DecisionEventCreate {
  idempotency_key?: string; trade_date: string; market: string;
  event_type: DecisionEventType; instrument_id?: string; plan_id?: string;
  candidate_id?: string; reason_code?: string; note?: string;
  discipline_version?: string; dataset_id?: string; facts_hash?: string;
...
export const getIndustryHeat = (tradeDate?: string) => req<IndustryHeatReport>(
  `/api/discipline/industry-heat${tradeDate ? `?trade_date=${encodeURIComponent(tradeDate)}` : ""}`,
);
export const getMarketEvidence = (tradeDate?: string) => req<MarketEvidenceReport>(
  `/api/discipline/market-evidence${tradeDate ? `?trade_date=${encodeURIComponent(tradeDate)}` : ""}`,
);
export const refreshIndustryWind = (tradeDate: string, topN = 8) =>
  jsonPost<IndustryHeatReport>(`/api/discipline/industry-heat/${encodeURIComponent(tradeDate)}/wind-refresh`, { top_n: topN });
...
export const getDecisionEvents = (filters: {
  tradeDate?: string; market?: string; instrumentId?: string; planId?: string; limit?: number;
} = {}) => {
  ...
  return req<DecisionEvent[]>(`/api/decision-events${query ? `?${query}` : ""}`);
};
export const createDecisionEvent = (event: DecisionEventCreate) =>
  jsonPost<DecisionEvent>("/api/decision-events", event);
...
// ── 美股右侧资产研究页（趋势事实，不生成交易指令） ──
export type UsRightSideFieldState =
  | "available" | "not_requested" | "not_returned" | "not_applicable" | "stale";
export type UsRightSideSortDirection = "asc" | "desc";
export interface UsRightSideCapabilities {
  enabled: boolean;
  scope: string;
  rules_version: string;
  research_only: true;
...
export interface UsRightSideRun {
  run_id: string;
  as_of_date: string;
  membership_as_of_date: string;
  upstream_update_dt: string | null;
  scope: string;
  rules_version: string;
  status: string;
  counts: {
    universe: number; scanned: number; right_side: number; unknown: number;
    age_covered?: number; strength_covered?: number; strength_target?: number;
    strength_max_days?: number; standard_covered: number; standard_target?: number;
    standard_max_days?: number | null; standard_top_n?: number;
    industries: number; industry_covered: number; deep_covered: number;
  };
  readiness: { age?: boolean; strength?: boolean; standard: boolean; industry: boolean; deep: boolean };
  cost: {
    estimated_screen_cny: string | null; approved_screen_cny: string | null; actual_screen_cny: string | null;
    estimated_age_cny?: string | null; approved_age_cny?: string | null; actual_age_cny?: string | null;
    estimated_strength_cny?: string | null; approved_strength_cny?: string | null; actual_strength_cny?: string | null;
    estimated_standard_cny: string | null; approved_standard_cny: string | null; actual_standard_cny: string | null;
    estimated_industry_cny: string | null; approved_industry_cny: string | null; actual_industry_cny: string | null;
    estimated_deep_cny: string | null; approved_deep_cny: string | null; actual_deep_cny: string | null;
    actual_total_cny: string | null; breakdown: Record<string, unknown>;
  };
  progress: Record<string, { completed_batches?: number; total_batches?: number }>;
  audit: {
    cache_key: { as_of_date: string; scope: string; universe_sha256: string; screen_fields_hash: string };
    field_sets: Record<string, { fields: string[] | null; sha256: string | null }>;
    manifest: { available: boolean; sha256: string | null };
...
export interface UsRightSideSortCapability {
  enabled: boolean;
  default_direction: UsRightSideSortDirection;
  blocked_reason: string | null;
}
export interface UsRightSideAsset {
  rank: number;
  tm_id: number;
  ticker_symbol: string;
  ticker_name: string | null;
  ...
  temperature_curr: string | null;
  days_since_trend_entry: number | null;
  gain_since_trend_entry: string | null;
  phase_curr: string | null;
  strength_local_curr: string | null;
  strength_local_change: string | null;
  industry_tm_id: number | null;
  industry_name: string | null;
  industry_temperature_curr: string | null;
  industry_strength_local_curr: string | null;
  industry_phase_curr: string | null;
  industry_is_right_side: boolean | null;
  industry_as_of_date: string | null;
  danger_flag: boolean | null;
  boiling_flag: boolean | null;
  champagne_flag: boolean | null;
  risk_flag_count: number | null;
  ticker_labels: string[];
  heat_score_7d: string | null;
  return_1m: string | null;
  field_states: Record<string, UsRightSideFieldState>;
  industry_field_states: Record<string, UsRightSideFieldState>;
  industry_source_method: string | null;
}
export interface UsRightSideOverview {
  capabilities: UsRightSideCapabilities;
  universe: {
    available: boolean; membership_as_of_date?: string; stock_count?: number;
    path?: string; sha256?: string; error?: { code: string; message: string };
  };
  run: UsRightSideRun | null;
  sort_capabilities: Record<string, UsRightSideSortCapability>;
}
export interface UsRightSideUniversePreflight {
  current: {
    available: boolean; membership_as_of_date?: string; stock_count?: number;
    path?: string; sha256?: string; error?: { code: string; message: string };
  };
  upstream: { tm_id: number | null; as_of_date: string; update_dt: string | null };
  billing_model: string | null;
  estimated_cost_cny: null;
  requires_unbounded_confirmation: true;
  note: string;
  free_evidence: {
    api_doc_sha256: string; change_log_sha256: string; billing_sha256: string;
    update_status_sha256: string; api_count: number | null; billing_field_count: number;
    latest_change: Record<string, unknown> | null; missing_required_fields: string[];
  };
}
export interface UsRightSideList {
  run: UsRightSideRun;
  items: UsRightSideAsset[];
  total: number;
  page_size: number;
  offset: number;
  next_cursor: string | null;
  sort: { sort_by: string; sort_dir: UsRightSideSortDirection };
  sort_capabilities: Record<string, UsRightSideSortCapability>;
  filters: Record<string, unknown>;
  facets: { industries: string[] };
}
export interface UsRightSidePreflight extends UsRightSideRun {
  preflight: {
    row_count: number; batch_count: number; fields: string[]; estimated_cost_cny: number;
    preflight_hash: string;
    batches: Array<{ batch_index: number; row_count: number; estimated_cost_cny: number }>;
  };
}
export interface UsRightSideListParams {
  runId?: string; query?: string; temperature?: string; phase?: string; industry?: string;
  daysMin?: number; daysMax?: number; strengthMin?: number; riskOnly?: boolean;
  sortBy?: string; sortDir?: UsRightSideSortDirection; cursor?: string | null; pageSize?: number;
}
export const getUsRightSideCapabilities = () =>
  req<UsRightSideCapabilities>("/api/us-right-side/capabilities");
export const getUsRightSideOverview = () =>
  req<UsRightSideOverview>("/api/us-right-side/overview");
export const preflightUsRightSideUniverse = () =>
  jsonPost<UsRightSideUniversePreflight>("/api/us-right-side/universe/preflight", {});
export const refreshUsRightSideUniverse = (approvedBudgetCny: string) =>
  jsonPost<{ cached: boolean; as_of_date: string; stock_count: number; path: string; actual_cost_cny: string | null }>(
    "/api/us-right-side/universe/refresh",
    { confirm_unbounded_all_basic: true, approved_budget_cny: approvedBudgetCny },
  );
export const preflightUsRightSideScan = () =>
  jsonPost<UsRightSidePreflight>("/api/us-right-side/scans/preflight", {});
export const runUsRightSideScan = (runId: string, approvedBudgetCny: string, preflightHash: string) =>
  jsonPost<UsRightSideRun>("/api/us-right-side/scans", {
    run_id: runId, approved_budget_cny: approvedBudgetCny, preflight_hash: preflightHash,
  });
export const preflightUsRightSideEnrichment = (
  runId: string, stage: "age" | "strength" | "standard" | "industry" | "deep",
) => jsonPost<UsRightSidePreflight>("/api/us-right-side/enrichment/preflight", {
  run_id: runId, stage, ...(stage === "strength" ? { max_days: 30 } : {}),
  ...(stage === "standard" ? { top_n: 100 } : {}),
});
export const runUsRightSideEnrichment = (
  runId: string, stage: "age" | "strength" | "standard" | "industry" | "deep", approvedBudgetCny: string, preflightHash: string,
) => jsonPost<UsRightSideRun>("/api/us-right-side/enrichment", {
  run_id: runId, stage, approved_budget_cny: approvedBudgetCny, preflight_hash: preflightHash,
});
export const getUsRightSideAssets = (params: UsRightSideListParams = {}) => {
  ...
  if (params.daysMin != null) query.set("days_min", String(params.daysMin));
  if (params.daysMax != null) query.set("days_max", String(params.daysMax));
  if (params.strengthMin != null) query.set("strength_min", String(params.strengthMin));
  if (params.riskOnly) query.set("risk_only", "true");
  if (params.sortBy) query.set("sort_by", params.sortBy);
  if (params.sortDir) query.set("sort_dir", params.sortDir);
  if (params.cursor) query.set("cursor", params.cursor);
  if (params.pageSize) query.set("page_size", String(params.pageSize));
  return req<UsRightSideList>(`/api/us-right-side/assets${query.size ? `?${query}` : ""}`);
};
export const getUsRightSideAsset = (runId: string, tmId: number) =>
  req<{ run: UsRightSideRun; asset: UsRightSideAsset }>(
    `/api/us-right-side/assets/${tmId}?run_id=${encodeURIComponent(runId)}`,
  );
export const getUsRightSidePlot = (runId: string, tmId: number) =>
  jsonPost<{ tm_id: number; cached: boolean; estimated_cost_cny: number; image: string }>(
    `/api/us-right-side/assets/${tmId}/plot`, { run_id: runId, approved_budget_cny: "0.10" },
  );
...
export const getUsCalculatorMarket = (ticker: string) =>
  req<UsCalculatorMarket>(`/api/us-calculator/market/${encodeURIComponent(ticker.trim().toUpperCase())}`);
...
export interface OkxMonitorCapabilities {
  read_only: true;
  trading_routes: false;
  supported_products: string[];
  policy_modes: Array<"auto_ema10" | "manual">;
  email_provider: string;
  monitor_enabled: boolean;
  shadow_mode: boolean;
}

export interface OkxProtectionSnapshot {
  order_key: string;
  order_type: string;
  side: string;
  quantity: string | null;
...
export interface OkxPositionPolicy {
  position_key: string;
  mode: "auto_ema10" | "manual";
...
export interface OkxPositionState {
  state: string;
  product_day: string | null;
  cooldown_until: string | null;
  reentry_frozen_line: string | null;
  reentry_attempts: number;
  consecutive_confirmed_bars: number;
}

export interface OkxPositionRow {
  position_key: string;
  captured_at: string;
  inst_id: string;
  inst_type: string;
  ...
  is_cash: boolean;
  policy: OkxPositionPolicy | null;
  state: OkxPositionState | null;
  protections: OkxProtectionSnapshot[];
}

export interface OkxMonitorEvent {
  event_id: number;
...
export interface OkxMonitorOverview {
  status: "ready" | "stale" | "awaiting_first_sync";
  stale?: boolean;
  heartbeat: {
    enabled: boolean; shadow_mode: boolean; status: string;
    last_tick_at: string | null; last_sync_at: string | null; error: string | null;
  } | null;
  latest_sync?: { sync_id: string; completed_at: string; position_count: number; protection_count: number };
  positions: OkxPositionRow[];
  cash: OkxPositionRow[];
  events: OkxMonitorEvent[];
}

export const getOkxMonitorCapabilities = () =>
  req<OkxMonitorCapabilities>("/api/okx-monitor/capabilities");
export const getOkxMonitorOverview = () =>
  req<OkxMonitorOverview>("/api/okx-monitor/overview");
export const getOkxPositionHistory = (positionKey: string) =>
  req<{ position_key: string; snapshots: OkxPositionRow[]; events: OkxMonitorEvent[] }>(
    `/api/okx-monitor/positions/${encodeURIComponent(positionKey)}/history?limit=50`,
  );
export const updateOkxPositionPolicy = (
  positionKey: string,
  body: { mode: "auto_ema10" | "manual"; manual_stop?: number | null; reason?: string | null },
) => req<OkxPositionPolicy>(`/api/okx-monitor/positions/${encodeURIComponent(positionKey)}/policy`, {
