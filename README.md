...
- 部署时必须设置 `TREND_DESK_ACCESS_KEY_SHA256`，即随机交易台访问密钥的 SHA-256 摘要；用户在网页输入原始的**交易台访问密钥**。
- 它不是 AI Builder 平台 API Key，也不是平台注入的 `AI_BUILDER_TOKEN`。
- 原始访问密钥只经本机剪贴板或密码管理器交给用户，绝不写入部署请求、日志或配置文件。
...
- `AI_BUILDER_TOKEN` 只由平台在服务端注入，仅用于服务端模型调用；不能作为网页登录密码。
- `TREND_DESK_ACCESS_KEY_SHA256` 可保存在部署平台的私有环境变量中；原始访问密钥不能提交到仓库或粘贴到聊天。
...
3. 用交易台访问密钥登录后，交易台、复盘和数据与规则页面可以打开。
